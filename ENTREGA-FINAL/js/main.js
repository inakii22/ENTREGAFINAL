...
const box = (titulo, imagen, precio) => {
    ...
    comprar.addEventListener('click', async () => {
        const { value: confirmarCompra } = await Swal.fire({
      ...
      cancelar.addEventListener('click', async () => {
        const { value: cancelarCompra } = await Swal.fire({
          title: 'Cancelar Carrito',
          text: '¿Estás seguro de que deseas cancelar el carrito?',
          icon: 'warning',
          showCancelButton: true,
        ...
        if (cancelarCompra) {
          ...
          Swal.fire({
            ...
            text: 'Tu carrito ha sido cancelado.',
            icon: 'info'
          });
        }
      });